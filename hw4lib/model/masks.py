...
def PadMask(padded_input, input_lengths):
    ...
    # mask = torch.zeros((batch_size, seq_length), dtype=torch.bool, device=padded_input.device)
    # for i in range(batch_size):
    #     mask[i, input_lengths[i]:] = True
    if input_lengths is None:
        # If no lengths provided, assume all positions are valid (no padding)
        return torch.zeros((batch_size, seq_length), 
                         dtype=torch.bool, 
                         device=padded_input.device)
    positions = torch.arange(seq_length, device=padded_input.device)
    positions = positions.expand(batch_size, seq_length)
    mask = positions >= input_lengths.unsqueeze(1)
    mask = mask.to(padded_input.device)  # Ensure mask is on the same device as input
    return mask
