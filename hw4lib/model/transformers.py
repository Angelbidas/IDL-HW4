...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        ])  # Use SelfAttentionDecoderLayer for each layer
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Use SelfAttentionEncoderLayer for each layer
        # Encoder layers should use the same d_model, num_heads, and d_ff as the decoder
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        '''
        #[01/01]    FAILED:   Test the encoder-decoder transformer
        ...
        ctc_logits_transposed = ctc_logits.transpose(0, 1)

        # TODO: Return the encoded representation, padding mask, running attention weights, and CTC inputs (see docstring)
        ctc_inputs = {'log_probs': ctc_logits_transposed, 'lengths': source_lengths}
    ...
    def decode(
        ...
        x_dec = self.target_embedding(padded_targets)  # Apply target embedding
        ...
        for i in range(self.num_decoder_layers):
            ...
            x_dec, self_attn, cross_attn = self.dec_layers[i](
                        x=x_dec,                         # Decoder input
                        enc_output=encoder_output,       # Encoder output for cross-attention
                        dec_key_padding_mask=pad_mask_tgt,  # For self-attention in decoder
                        enc_key_padding_mask=pad_mask_src,  # For cross-attention with encoder output
                        attn_mask=causal_mask            # Causal mask for self-attention
                    )
