...
class SelfAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dropout: float = 0.0):
        ...
        self.mha = nn.MultiheadAttention(
            ...
            num_heads=num_heads,
            dropout=dropout,
            batch_first=True  # Input is (batch_size, seq_len, d_model)
        )
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # NOTE: The attention weights are needed for visualization
        return x, mha_attn_weights  # Return the output tensor and attention weights
...
class CrossAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dropout: float = 0.0):
        ...
        # TODO: Initialize the multi-head attention mechanism (use nn.MultiheadAttention)
        # NOTE: The key and value inputs will be from the encoder
        # NOTE: The query input will be from the decoder
        # NOTE: Use batch_first=True to match the input shape (batch_size, seq_len, d_model)
        # NOTE: You can use the same parameters as in SelfAttentionLayer
        self.mha = nn.MultiheadAttention(
            embed_dim=d_model,
            num_heads=num_heads,
            dropout=dropout,
...
class FeedForwardLayer(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, dropout: float = 0.0):
        ...
        # NOTE: The feed-forward network should consist of:
        # 1. First linear layer: d_model -> d_ff
        # 2. GELU activation
        # 3. Dropout
        # 4. Second linear layer: d_ff -> d_model
        
        # Initialize the normalization layer
        self.norm = nn.LayerNorm(d_model)
        
        # Initialize the dropout layer
        self.dropout = nn.Dropout(dropout)  # Initialize the dropout layer
