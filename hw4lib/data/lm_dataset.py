...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        self.text_dir = os.path.join(config.get("root", "data"), partition)
        ...
        if subset_size is not None and subset_size > 0 and subset_size < len(self.text_files):
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        lengths = torch.LongTensor([len(transcript) for transcript in shifted_transcripts])  # (B)
