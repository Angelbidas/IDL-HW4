...
class LMTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            # Unpack batch correctly - make sure to use the same unpacking pattern used in your dataloader
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                # Pass target_lengths to the model - this is what's missing in your code
                raw_preds, attn_weights = self.model(targets_shifted, lengths)
                

                # Calculate loss
                _, _, vocab_size = raw_preds.shape
                raw_loss = self.criterion(raw_preds.reshape(-1, vocab_size), targets_golden.reshape(-1))
    ...
    def _validate_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.inference_mode():
                # Get raw predictions and attention weights from model
                raw_preds, attn_weights = self.model(targets_shifted, lengths)
                ...
                _, _, vocab_size = raw_preds.shape
                loss = self.criterion(raw_preds.reshape(-1, vocab_size), targets_golden.reshape(-1))
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                seqs, scores = generator.generate_sample(prompts, 
                ...
                seqs, scores = generator.generate_beam(prompts,
                                                        beam_width=generation_config.get('beam_width', 10),
                                                        temperature=generation_config.get('temperature', 1.0),
